...
const enableResize = 0; // 0 = off, 1 = on - Controls automatic tab switching and textarea expansion on resize/mobile
document.querySelector("#asset-updated").textContent =
  `These are populated when an asset number is searched. The asset data was last updated on ${dataDate}.`;
document.querySelector("#user-local-storage").textContent =
  `User information is saved in the browser's local storage by completing the fields. Clear the browser cache to remove.`;
...
function handleTabClick(event) {
  ...
  if (target) {
    ...
    if (target === "review-summary-tab") {
      document.getElementById("asset-updated").textContent =
        `The summary is built automatically when an asset number is searched. The asset data was last updated on ${dataDate}.`;
    }
    // If "review-ratings-tab" is clicked, clear the asset-updated message
    ...
    if (target === "settings-global-tab") {
      document.querySelector("#user-local-storage").textContent =
        `Global features are under development.`;
    ...
    if (target === "settings-guide-tab") {
      document.querySelector("#user-local-storage").textContent =
        `A general overview of site functionality.`;
    }
...
function checkScreenWidthAndTriggerClicks() {
  if (!enableResize) return; // Skip if resize functionality is disabled
...
// :::: (Comment Builder Tab Open in JS so Close in JS)

function openCommentBuilder() {
  window.open("https://sfhurst.github.io/comment_builder/", "_blank");
}
